...
ORDERS_DB = {
    ...
    "ORD124": {"status": "processing", "total": 149.99, "customer": "jane@email.com"},
    "ORD456": {"status": "processing", "total": 50.00, "customer": "test@email.com"},
...
def check_order_status(order_id:str)->dict:
    """
    Checks the current status of a customers order. 
...
def process_refund(order_id:str,reason:str)->dict:
    ...
    Args:
        ...
        reason (str): Customers reason for refund.
...
def escalate_to_supervisor(issue_summary: str, order_id: str) -> dict:
    ...
    Use this when you cannot resolve the customer's issue with available tools or when the customer explicitly requests to speak with a supervisor.
...
root_agent = Agent(
    ...
    2. You MUST ALWAYS FIRST use check_order_status to verify the order exists and check its current status.
    3. If order not found, cannot proceed with refund - ask customer to verify order ID.
    4. If order exists, you MUST then use process_refund with the order_id and customer's reason.
    ...
    - Offer to escalate to supervisor if they want an exception. ALWAYS offer this for cannot_refund errors.
